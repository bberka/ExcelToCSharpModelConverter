...
internal static class XBuilderHelper
{
  ...
  internal static void AppendClassStart(StringBuilder sb, string className, bool isSheet) {
    if (isSheet) {
      ...
      if (XOptionLib.This.Option.SheetModelInheritanceList.Count > 0) sb.Append("," + string.Join(",", XOptionLib.This.Option.SheetModelInheritanceList));
    }
    else {
      sb.Append("public class " + className + " : XFileBase");
      if (XOptionLib.This.Option.FileModelInheritanceList.Count > 0) sb.Append("," + string.Join(",", XOptionLib.This.Option.FileModelInheritanceList));
    }
